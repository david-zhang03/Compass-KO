...
def wilcoxon_test(consistencies_matrix, group_A_cells, group_B_cells):
	...
	#per reaction/meta-reaction, conduct wilcoxon test between group_A and group_B
        # Check if same indicies (i.e. comparing same DTP cells across two different conditions) 
        if group_A_cells == group_B_cells: 
            group_A_cells = [id + "_x" for id in group_A_cells] 
            group_B_cells = [id + "_y" for id in group_B_cells] 
